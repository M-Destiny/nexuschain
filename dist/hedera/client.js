...
export class HederaClient {
     ...
     */
    async fundFromTestnetFaucet() {
        if (this.network !== 'testnet') {
            throw new Error(`fundFromTestnetFaucet is only safe on testnet; current network is '${this.network}'.`);
        }
    ...
    // ---------------------------------------------------------------------------
    async executeWithBreaker(op, opName) {
        try {
    ...
    assertCircuitClosed() {
        ...
        if (this.circuit.openUntil > now) {
            ...
            throw new Error(`HederaClient circuit breaker is OPEN (${this.circuit.failures} consecutive failures). ` +
    ...
    recordFailure(opName, err) {
        ...
        if (this.circuit.failures >= this.circuitFailureThreshold) {
            ...
            console.warn(`[HederaClient] circuit OPEN: ${this.circuit.failures} consecutive failures. ` +
