...
export class AgentRegistry {
  ...
  async registerAgent(metadata: Omit<AgentMetadata, 'id'>, _privateKey: string): Promise<string> {
    const id = uuid();
    const agent: AgentMetadata = { id, ...metadata, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
  ...
  async updateAgent(agentId: string, updates: Partial<AgentMetadata>): Promise<void> {
    ...
    const updated: AgentMetadata = { ...existing, ...updates, updatedAt: new Date().toISOString() };
  ...
  async listAgents(filters?: AgentFilter): Promise<AgentMetadata[]> {
   ...
   */
  async upgradeAgent(agentId: string, newVersion: string, newIpfsCid: string): Promise<void> {
    const agent = this.agents.get(agentId);
    if (!agent) throw new Error(`Agent ${agentId} not found`);
